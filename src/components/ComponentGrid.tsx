...
export function ComponentGrid({ components }: Props) {
  return (
    <section className="mt-10 rounded-3xl bg-white shadow-soft border border-slate-100 overflow-hidden animate-slide-up" style={{ animationDelay: '120ms' }}>
      ...
      <ul className="divide-y divide-slate-100">
        {components.map((c) => (
          <li key={c.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50/60 transition-colors">
            <span className={`shrink-0 h-2 w-2 rounded-full ${STATUS_DOT[c.status]}`} aria-hidden="true" />
            ...
            <span className={`shrink-0 font-mono text-xs font-medium ${STATUS_TEXT[c.status]}`}>
